...
def EDA_processing(df,response,expected_schema=None):
 ...
 # Convert binary classification target to 0/1 only when needed
    if response in df.columns:
        unique_vals = df[response].dropna().unique()

        if len(unique_vals) == 2:
            print(f"\nTarget variable '{response}' detected as binary.")
            print(f"  Unique values: {unique_vals}")

            # Case 1: Already 0 and 1
            if set(unique_vals) == {0, 1}:
                print("  Already encoded as 0 and 1. No mapping needed.")

            # Case 2: Boolean values
            elif df[response].dtype == 'bool' or set(unique_vals) == {True, False}:
                df[response] = df[response].astype(int)
                print("  Boolean detected. Converted True→1, False→0.")

            # Case 3: Categorical binary labels
            else:
                val1, val2 = unique_vals[0], unique_vals[1]
                try:
                    df[response] = df[response].map({val1: 1, val2: 0})
                    print(f"  Mapping done: {val1} → 1, {val2} → 0")
                except Exception as e:
                    print(f" Failed to map binary values. Error: {e}")
    ...
    print("Top 5 Rows of Datasets After EDA Processing :")
    print(df.head())
    # Correlation Matrix
...
def evaluate_linear_regression(x,y,predictors):
    ...
    print("Plotting of Predicted vs Actual")
    plot_predictions_vs_actuals(train_y,y_train_pred,title="Linear Regression (Predicted vs Actual on Train Data)")
    plot_predictions_vs_actuals(test_y,y_test_pred,title="Linear Regression (Predicted vs Actual on Test Data)")
...
def best_lambda(x,y,lambda_values=np.logspace(-4,4,100),k=10):
    avg_cv_errors=[]
...
def evaluate_ridge_regression(x,y,predictors,k=10):
    train_x, test_x ,train_y,test_y= train_test_split(x,y, test_size=0.3, random_state=42)
    # Standardise the train Data
    x_mean=np.mean(train_x,axis=0)
    x_std=np.std(train_x,axis=0)
    x_standardized = (train_x - x_mean) / x_std
    best_lam=best_lambda(x_standardized,train_y,k=k)
    ...
    print(f"Best Lambda for Ridge Regression: {best_lam:.4f}")
    ...
    print(f"Test Adjusted R^2 :{test_adj_r2 :.2f}")
    # Plotting Between Test Actual vs Test Predicted
    plot_predictions_vs_actuals(train_y,y_train_pred,title="Ridge Regression (Predicted vs Actual on Train Data)")
    plot_predictions_vs_actuals(test_y,y_test_pred,title="Ridge Regression (Predicted vs Actual on Test Data)")
...
def best_lambda_lasso(x,y,lambda_values=np.logspace(-4,4,100),k=10):
    avg_cv_errors=[]
    kf = KFold(n_splits=k, shuffle=True, random_state=42)
    for lam in lambda_values:
        cv_errors = []
...
def evaluate_lasso_regression(x,y,predictors,k=10,max_iter=1000):
    train_x, test_x ,train_y,test_y= train_test_split(x,y, test_size=0.3, random_state=42)
    ...
    best_lam=best_lambda_lasso(x_standardized,y_centre,k=k)
    final_beta=lasso_coordinate_descent(x_standardized,y_centre,best_lam,max_iter=max_iter)
    ...
    # print the best lambda
    print(f"Best Lambda for Lasso Regression: {best_lam:.4f}")
    # Display the coefficients
    ...
    print(f"Test Adjusted R^2 :{test_adj_r2 :.2f}")
    # Plotting Between Test Actual vs Test Predicted
    plot_predictions_vs_actuals(train_y,y_train_pred,title="Lasso Regression (Predicted vs Actual on Train Data)")
    plot_predictions_vs_actuals(test_y,y_test_pred,title="Lasso Regression (Predicted vs Actual on Test Data)")
...
def evaluate_regression_tree(data, predictors, alpha_values=np.logspace(-4, 0, 50), test_size=0.3, random_state=42, min_samples_split=20,max_depth=8):
    # Train/Test Split
    ...
    plot_predictions_vs_actuals(y_train,y_train_pred,title="Regression Tree (Predicted vs Actual on Train Data)")
    plot_predictions_vs_actuals(y_test,y_test_pred,title="Regression Tree (Predicted vs Actual on Test Data)")
...
def evaluate_random_forest(data,predictors,n_tress=10,max_depth=5,max_features=4):
    ...
    plot_predictions_vs_actuals(train_y,train_preds,title="Random Forest (Predicted vs Actual on Train Data)")
    plot_predictions_vs_actuals(test_y,test_preds,title="Random Forest (Predicted vs Actual on Test Data)")
...
def evaluate_XGBoost(data,predictors,n_estimators=100,max_depth=2):
    ...
    print(f"Train Adjusted R^2 :{train_adj_r2 :.2f}")
    print(f"Test Adjusted R^2 :{test_adj_r2 :.2f}")
    plot_predictions_vs_actuals(y_train,train_preds,title="XGBoost (Predicted vs Actual on Train Data)")
    plot_predictions_vs_actuals(y_true,test_preds_xg,title="XGBoost (Predicted vs Actual on Test Data)")
